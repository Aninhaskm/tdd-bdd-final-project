...
class TestProductModel(unittest.TestCase):
    ...
    def test_update_without_id_raises(self):
        """Deve lançar exceção ao tentar atualizar produto sem ID"""
        product = ProductFactory()
        product.id = None
    ...
    def test_deserialize_with_invalid_available_type(self):
        """Deve lançar exceção se o tipo de 'available' for inválido durante desserialização"""
        data = {
            "name": "Test",
            "description": "Desc",
            "price": "10.0",
    ...
    def test_deserialize_missing_key(self):
        """Deve lançar uma exceção se uma chave obrigatória estiver ausente no desserializar"""
        data = {
            "description": "Desc",
            "price": "10.0",
            "available": True,
    ...
    def test_find_by_price_with_string(self):
        """Deve converter string para Decimal ao buscar por preço"""
        price_str = "19.90"
        products = ProductFactory.create_batch(3)
        for prod in products:
            prod.price = Decimal(price_str)
            prod.create()

        found = Product.find_by_price(price_str)
        for product in found:
            self.assertEqual(str(product.price), price_str)

    def test_deserialize_with_invalid_attribute(self):
        """Deve lançar exceção se um atributo inválido for passado para desserialização"""
        data = {
            "name": "Wrench",
    ...
    def test_deserialize_missing_price(self):
        """Deve lançar exceção se o campo 'price' estiver ausente na desserialização"""
        data = {
            "name": "Wrench",
            "description": "Heavy tool",
            "available": True,
    ...
    def test_deserialize_with_type_error(self):
        """Deve lançar exceção se os dados fornecidos para desserializar forem inválidos"""
        product = Product()
        with self.assertRaises(DataValidationError) as context:
